...
final class DirectoryExplanationViewController: BaseApiKeyViewController {
    ...
    private func setUpUI() {
        ...
        openingAttributedString.addAttributes(
            [.foregroundColor: SLColor.textColor, .font: UIFont.systemFont(ofSize: 15)],
            range: NSRange(openingText.startIndex..., in: openingText))

        if let onTheFlyRange = openingText.range(of: "on the fly") {
            openingAttributedString.addAttribute(.font,
                                                 value: UIFont.boldSystemFont(ofSize: 15),
                                                 range: NSRange(onTheFlyRange, in: openingText))
        }
        ...
        listAttributedString.addAttributes(
            [.foregroundColor: SLColor.textColor, .font: UIFont.systemFont(ofSize: 15)],
        ...
        if let firstMatchRange = listText.range(of: "your_directory/anything@\(firstDomain)"),
            ...
            listAttributedString.addAttribute(.backgroundColor,
                                              value: backgroundColor,
                                              range: NSRange(firstMatchRange, in: listText))
            listAttributedString.addAttribute(.backgroundColor,
                                              value: backgroundColor,
                                              range: NSRange(secondMatchRange, in: listText))
            listAttributedString.addAttribute(.backgroundColor,
                                              value: backgroundColor,
                                              range: NSRange(thirdMatchRange, in: listText))
        }
        ...
        closingAttributedString.addAttributes(
            [.foregroundColor: SLColor.textColor, .font: UIFont.systemFont(ofSize: 15)],
            range: NSRange(closingText.startIndex..., in: closingText))

        if let firstMatchRange = closingText.range(of: "anything"),
            let secondMatchRange = closingText.range(of: "your_directory") {
            let backgroundColor = UIColor.systemYellow
            closingAttributedString.addAttribute(.backgroundColor,
                                                 value: backgroundColor,
                                                 range: NSRange(firstMatchRange, in: closingText))
            closingAttributedString.addAttribute(.backgroundColor,
                                                 value: backgroundColor,
                                                 range: NSRange(secondMatchRange, in: closingText))
        }

        if let anythingRange = closingText.range(of: "anything") {
            closingAttributedString.addAttribute(.font,
                                                 value: UIFont.boldSystemFont(ofSize: 15),
                                                 range: NSRange(anythingRange, in: closingText))
        }
        ...
        domainListAttributedString.addAttributes(
            [.foregroundColor: SLColor.textColor, .font: UIFont.systemFont(ofSize: 15)],
            range: NSRange(domainListText.startIndex..., in: domainListText))

        userOptions.domains.forEach { domain in
            if let range = domainListText.range(of: domain) {
                domainListAttributedString.addAttribute(.font,
                                                        value: UIFont.boldSystemFont(ofSize: 15),
                                                        range: NSRange(range, in: domainListText))
            }
        }
