...
final class DirectoryViewController: BaseApiKeyLeftMenuButtonViewController, Storyboarded {
    ...
    @IBAction private func addButtonTapped() {
        let alert = UIAlertController(title: "Create directory",
                                      message: "Pick an epic name for your directory",
    ...
    private func showConfirmDeletionAlert(directory: Directory) {
        let alert = UIAlertController(
            ...
            message: "All aliases associated with \"\(directory.name)\" directory will be also deleted",
            preferredStyle: .alert)

        let deleteAction =
            UIAlertAction(title: "Yes, delete this directory", style: .destructive) { [unowned self] _ in
