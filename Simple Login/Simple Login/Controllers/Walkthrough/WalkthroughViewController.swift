...
final class WalkthroughViewController: UIPageViewController, Storyboarded {
    private var currentWalkthroughStepVCIndex: Int = 0
    ...
    override func viewDidLoad() {
        ...
        if let walkthroughStep1ViewController = walkthroughStepControllerAt(index: 0) {
            setViewControllers([walkthroughStep1ViewController],
                               direction: .forward,
                               animated: true)
        }
    }
    ...
    private func walkthroughStepControllerAt(index: Int) -> BaseWalkthroughStepViewController? {
        ...
        case 3:
            let walkthroughLastStepViewController =
                WalkthroughLastStepViewController.instantiate(storyboardName: "Walkthrough")
...
extension WalkthroughViewController: UIPageViewControllerDataSource {
    ...
    func presentationIndex(for pageViewController: UIPageViewController) -> Int {
        currentWalkthroughStepVCIndex
    }

    func pageViewController(_ pageViewController: UIPageViewController,
                            viewControllerBefore viewController: UIViewController) -> UIViewController? {
        guard let walkthroughStepController =
                viewController as? BaseWalkthroughStepViewController else { return nil }

        let index = walkthroughStepController.index
        currentWalkthroughStepVCIndex = index
